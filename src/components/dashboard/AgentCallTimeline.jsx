...
export default function AgentCallTimeline({ calls: incomingCalls = [] }) {
  ...
  return (
    <div style={{ width: '100%', overflow: 'hidden' }}>
      {/* Legend */}
      <div className="flex items-center gap-3 flex-wrap mb-5 mt-1 ml-6 mr-5">
        ...
        <div key={e.name} className="flex items-center gap-1">
            ...
            <span style={{ fontSize: 10, color: textSecondary, fontWeight: 600 }} className="font-thin text-base">{e.name}</span>
          </div>
        )}
      ...
      {AGENTS.map((agent) => {
        ...
        return (
          <div key={agent} style={{ display: 'flex', alignItems: 'center', height: ROW_H, marginBottom: 4 }}>
            ...
            {/* Lane */}
            <div style={{ flex: 1, minWidth: 0, height: '100%', position: 'relative', background: laneBg, borderRadius: 8, border: `1px solid ${laneBorder}`, overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: GAP, padding: '4px 0' }} className="rounded-[64px] opacity-95">

// eQuo — unresolved alerts, shown above everything for admins. Now supports
// mood_streak alerts alongside flagged / low_rating. Warm amber / soft red.
import React from "react";
import { motion, AnimatePresence } from "framer-motion";
import { AlertTriangle, Flag, Check, Heart } from "lucide-react";
...
function reasonMeta(reason) {
  ...
  if (reason === "mood_streak") return { label: "Low mood streak", icon: Heart };
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    <span
                      ...
                      style={{ background: a.reason === "flagged" ? t.softRedDeep : a.reason === "mood_streak" ? t.violetDeep : t.amberDeep }}
                    >
                      <Icon className="w-3 h-3" /> {meta.label}
                    </span>
                    <span className="text-sm font-bold" style={{ color: t.text }}>{a.respondent_name || a.respondent_email}</span>
                    {typeof a.rating === "number" && (
                      <span className="text-xs font-bold" style={{ color: t.softRedDeep }}>
                        · {a.alert_type === "mood" ? `mood ${a.rating}/5` : `rated ${a.rating}/10`}
                      </span>
                    )}
                  </div>
                  {a.question_text && (

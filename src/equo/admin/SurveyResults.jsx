// eQuo — all published surveys with per-question average rating and
// respondent count, expandable per survey. Respects anonymity in drill-down.
...
function SurveyRow({ survey, questions, responses }) {
  ...
  const ratedResponses = surveyResponses.filter((r) => r.rating != null);
  const respondents = new Set(surveyResponses.map((r) => r.respondent_email)).size;
  const overallAvg = ratedResponses.length
    ? Math.round((ratedResponses.reduce((a, r) => a + r.rating, 0) / ratedResponses.length) * 10) / 10
    : 0;
      ...
      <button onClick={() => setOpen((o) => !o)} className="w-full flex items-center gap-4 text-left">
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs" style={{ color: t.textSoft }}>
            {survey.publish_date} · {surveyQuestions.length} questions
            {survey.is_anonymous && <span className="ml-2 text-[10px] font-bold" style={{ color: t.violet }}>· Anonymous allowed</span>}
          </div>
        </div>
